...
df
df <- subset(df, followup == 1) # subset 1-month timepoint data
df <- df %>%
...
model.out$OddsRatio$Median_CI # check for 1 to see which trt is more effective or differs significantly
model.out$TrtRankProb # ranks the trt 
...
absolute.plot(model.out, width = 5, height = 1.5, 
              network.name="1mo", alphabetic = FALSE)

# treatment-specific relative effect, pairwise 95% CI contrast plot
contrast.plot(model.out, reference = "SHAM", width = 5, height = 1.5, 
              network.name="1mo",
              digit=2) 

# rank probability 
rank.prob(model.out, cex.axis = 1, cex.lab = 2)
title("1-Month Rank Probability")
...
write.csv(final_df, "1mo_final_data.csv", row.names = FALSE)
