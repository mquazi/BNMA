...
df
df <- subset(df, followup == 6) # subset 6-month timepoint data
df <- df %>%
...
## network plot ##
nma.networkplot(studies, Treatment, data = df, title = "6-Month",
...
absolute.plot(model.out, width = 5, height = 1.5, 
              network.name="6mo", alphabetic = FALSE)
...
title("6-Month Rank Probability")
...
write.csv(final_df, "6mo_final_data.csv", row.names = FALSE)
