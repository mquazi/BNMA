...
df
df <- subset(df, followup == 3)
df <- df %>%
  filter(!is.na(X50oroverpainreduction_n))
df <- df %>% filter(Treatment != "SHAM") # dropping because SHAM is 0/18 for n and N
result <- df %>%
  group_by(studies) %>%
...
## network plot ##
nma.networkplot(studies, Treatment, data = df, title = "3-Month",
...
absolute.plot(model.out, width = 5, height = 1.5, 
              network.name="3mo", alphabetic = FALSE)
...
title("3-Month Rank Probability")
...
write.csv(final_df, "3mo_final_data.csv", row.names = FALSE)
